...
const severityConfig = {
  ...
  low: {
    color: "text-cyan-500",
    bg: "bg-cyan-950/20 border-cyan-900/50",
    icon: Info,
  },
};
...
const Toast: React.FC<{
  ...
  const config = {
    error: {
      bg: "bg-red-950/90 border-red-800",
      icon: XCircle,
      iconColor: "text-red-400",
    },
    warning: {
      bg: "bg-yellow-950/90 border-yellow-800",
      icon: AlertTriangle,
      iconColor: "text-yellow-400",
    },
    info: {
      bg: "bg-cyan-950/90 border-cyan-800",
      icon: Info,
      iconColor: "text-cyan-400",
    },
    success: {
      bg: "bg-green-950/90 border-green-800",
      icon: CheckCircle2,
...
const AIInsightCard: React.FC<{
  ...
  const getStatusGradient = (emoji: string) => {
    if (emoji === "🔴" || emoji === "🚨")
      return "from-red-950/40 via-slate-900/60 to-slate-900/80 border-red-900/50";
    if (emoji === "🟠")
      return "from-orange-950/40 via-slate-900/60 to-slate-900/80 border-orange-900/50";
    if (emoji === "🟡")
      return "from-yellow-950/30 via-slate-900/60 to-slate-900/80 border-yellow-900/40";
        ...
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            ...
            <p className="text-xs text-slate-600">
              Connecting to your security brain...
            </p>
          </div>
        </div>
  ...
  return (
    <div
      className={`bg-linear-to-br ${getStatusGradient(
        statusEmoji
      )} border rounded-2xl p-8 relative overflow-hidden`}
    >
        ...
        <div className="mb-4">
          ...
          <div className="flex flex-wrap gap-3">
            {decisions.map((decision) => {
              const isExecuting =
                activeButton === decision.id || isExecutingAction;
              return (
                <button
                  key={decision.id}
                  onClick={() => handleAction(decision)}
                  disabled={isExecuting}
                  className={`px-5 py-2.5 rounded-lg font-medium text-sm transition-all duration-200 border ${getButtonStyle(
                    decision.severity
                  )} ${
                    isExecuting
...
const App: React.FC = () => {
  ...
  const handleSecurityAction = useCallback(
    async (action: { id: string; action_type: string; target?: string }) => {
      if (action.action_type === "expand") return;
      try {
        const res = await axios.post(
          `${ORACLE_URL}/api/actions/execute`,
          {
            action_id: action.id,
        ...
        if (res.data.success) {
          ...
          setActionToast({
            message: `Action failed: ${res.data.message}`,
            type: "error",
          });
        }
      } catch (err) {
              ...
              <div className="flex flex-col gap-6">
                <div className="bg-slate-900/40 border border-slate-900 p-6 rounded-xl flex-1 flex flex-col justify-center relative overflow-hidden">
                  ...
                  {isConnected && data ? (
                    <>
                      <p
                        className={`text-5xl font-extralight relative z-10 ${
                          (data?.risk_score || 0) >= 0.7
                            ? "text-red-400"
                            : (data?.risk_score || 0) >= 0.4
                            ? "text-yellow-400"
                    ...
                    <p className="text-5xl font-extralight text-slate-700 relative z-10">
                      —
                    </p>
                  )}
                </div>
                        ...
                        return (
                          ...
                          >
                            ...
                            <td className="px-6 py-5">
                              ...
                              <p className="text-xs text-slate-500 mt-0.5 line-clamp-1 italic max-w-md">
                                "{alert.description}"
                              </p>
                            </td>
                            ...
                            <td className="px-6 py-5 text-right">
                              <span
                                className={`inline-flex items-center gap-1.5 text-[9px] font-black px-2.5 py-1 rounded border tracking-tighter ${config.bg} ${config.color}`}
                              >
