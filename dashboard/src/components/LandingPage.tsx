  ...
  return (
    <div ref={containerRef}>
      ...
      >
        ...
        <div className="flex-1 flex flex-col items-center justify-center px-4 text-center relative z-10" style={{ marginBottom: '120px' }}>
          ...
          <div className="animate-fade-in" style={{ animationDelay: '0.6s' }}>
            {/* FIX: Updated class names to canonical forms (w-105, md:w-120) */}
            <div className="relative h-16 w-105 md:w-120 mx-auto" style={{ perspective: '1000px' }}>
      ...
      >
        ...
        <div 
          // FIX: Updated class names to canonical forms (min-h-150)
          className={`relative z-10 w-full px-8 md:px-16 lg:px-24 transition-all duration-1000 min-h-150 flex items-center ${featuresVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-12'}`}
        >
        ...
        <div className="relative min-h-screen flex flex-col">
          ...
          >
            <div 
              key={activeTab}
              // FIX: Updated class names to canonical forms (min-h-150)
              className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-center min-h-150 animate-fade-in-up"
            >
                      ...
                      <div className="space-y-0">
                        {(useCaseTabs[activeTab].visual.data as Array<{time: string, event: string, status: string}>).map((item, idx) => (
                          <div key={idx} className="flex items-start gap-4 pb-4 relative">
                            {idx < 3 && (
                              // FIX: Updated class names to canonical forms (left-1.75)
                              <div className="absolute left-1.75 top-6 w-0.5 h-full bg-gray-700" />
                            )}
                            {/* FIX: Updated class names to canonical forms (shrink-0) */}
